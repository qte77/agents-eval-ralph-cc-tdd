...
class GraphMetrics(BaseModel):
    ...
    avg_closeness_centrality: float = Field(..., ge=0.0, le=1.0)
    num_nodes: int
    num_edges: int
...
class GraphComplexityAnalyzer:
    ...
    def calculate_clustering_coefficient(self, graph: nx.DiGraph) -> float:
        ...
        # nx.clustering returns dict[node, float] but pyright struggles with the type
    ...
    def calculate_closeness_centrality(self, graph: nx.DiGraph) -> float:
        ...
        Returns:
            Average closeness centrality between 0.0 and 1.0
        """
        if graph.number_of_nodes() == 0:
            return 0.0

        # For directed graphs with disconnected components, handle each component
        centrality_values = []
        for component in nx.weakly_connected_components(graph):
            subgraph = graph.subgraph(component)
            if subgraph.number_of_nodes() > 1:
                centrality = nx.closeness_centrality(subgraph)
                centrality_values.extend(centrality.values())

        if not centrality_values:
            return 0.0

        return sum(centrality_values) / len(centrality_values)
    ...
    def calculate_metrics(self, interactions: list[AgentInteraction]) -> GraphMetrics:
        ...
        return GraphMetrics(
            ...
            avg_closeness_centrality=self.calculate_closeness_centrality(graph),
